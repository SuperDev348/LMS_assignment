...
const columns = [
  ...
  { id: 'authorTitle', label: 'Author Title', minWidth: 100 },
  {
    id: 'action',
    label: 'Action',
    minWidth: 170,
...
const Testimonial = () => {
  ...
  return (
    <Paper className={classes.root}>
      ...
      <div className={classes.title}>Testimonial Setting</div>
      ...
      <TableContainer className={classes.container} style={{fontSize: 15}}>
          <Table stickyHeader aria-label="sticky table">
            ...
            <TableBody>
              {testimonials.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => {
                return (
                      ...
                      return (
                        <TableCell key={column.id} align={column.align} style={{fontSize: 14}}>
                          {column.id === 'action'?
                            (
                              <>
                                <Edit testimonial={row} refresh={refresh} />
                                <Delete testimonial={row} refresh={refresh} />
                              </>
                            ):
                            column.id === 'avatar'?
                            <img src={row.avatar} style={{height: 60}} />:
                            value
                          }
                        </TableCell>
                      );
